...
export function NotificationBell({ className }: { className?: string }) {
  ...
  return (
    ...
    >
      ...
      {unread > 0 && (
        <span className="absolute top-1 right-1 min-w-[16px] h-4 flex items-center justify-center rounded-full bg-violet-600 text-[10px] font-bold text-white px-1 leading-none">
          {unread > 99 ? '99+' : unread}
        </span>
      )}
    </Link>
