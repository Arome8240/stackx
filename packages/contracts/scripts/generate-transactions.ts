...
async function main() {
  ...
  for (let i = 0; i < randomAddresses.length; i++) {
    const amount = ethers.parseUnits((Math.random() * 100 + 1).toFixed(2), 18);
    const tx = await token.transfer(randomAddresses[i], amount);
    await tx.wait();
    console.log(`   Transfer ${i + 1}: ${ethers.formatUnits(amount, 18)} tokens to ${randomAddresses[i].slice(0, 10)}...`);
  ...
  const batchAmounts = randomAddresses.map(() => 
    ethers.parseUnits((Math.random() * 50 + 1).toFixed(2), 18)
  );
  ...
  await mintTx.wait();
  console.log(`   Minted ${ethers.formatUnits(mintAmount, 18)} tokens`);
  ...
  await burnTx.wait();
  console.log(`   Burned ${ethers.formatUnits(burnAmount, 18)} tokens`);
  console.log(`   Tx hash: ${burnTx.hash}`);

  // 5. Approve transactions
  console.log("\n5. Creating approvals...");
  for (let i = 0; i < 3; i++) {
    const approveAmount = ethers.parseUnits((Math.random() * 1000 + 100).toFixed(2), 18);
    const approveTx = await token.approve(randomAddresses[i], approveAmount);
    await approveTx.wait();
    console.log(`   Approval ${i + 1}: ${ethers.formatUnits(approveAmount, 18)} tokens for ${randomAddresses[i].slice(0, 10)}...`);
